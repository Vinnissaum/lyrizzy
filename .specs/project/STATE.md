...
**Current phase:** Phase 14 SPECIFIED (2026-08-11) — `.specs/features/phase14-multiscreen-liveedit-camera/spec.md` (30 reqs P14-01..P14-30) across three groups: 14A multi-screen launch modal + three-value launch policy + per-monitor naming (D-53, D-54); 14B live lyrics editing with section-anchored slide regeneration (D-55); 14C per-camera stream profiles replacing the rejected "camera resolution" request (D-56, D-57). Design DONE (2026-08-11) — `design.md` resolves all 5 design questions (DQ-1..DQ-5) against the code and records D-58..D-60. Tasks DONE (2026-08-11) — `tasks.md` breaks the phase into **22 tasks** across four slices (14C: T1–T6, 14B: T7–T11, 14A: T12–T20, 14D icon: T22, wrap-up: T21), all 32 requirements mapped, three pre-approval checks green. Slice **14D added 2026-08-11** on user request: app-icon rebrand to the Lyrizzy "L" whose stroke doubles as a music-note stem (D-64), spec'd as P14-31/P14-32. Three design refinements found while planning (R-1 anchor helpers move to `domain/slide.rs` for matrix-compliant unit tests; R-2 regeneration split into a pool-only core so `tests/presentation.rs` can test it without `AppHandle`; R-3 new task T6 — `update_set_item` currently patches only `OutputId::One`, so a camera on Tela 2 would never see a profile switch). Baseline measured 2026-08-11: **307 Rust / 480 Vitest** green. No code changes yet.
