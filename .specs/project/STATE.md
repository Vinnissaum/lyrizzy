...
| D-79 | `v1.3.2` reconciles line-ending-shifted migration checksums at startup rather than reverting `.gitattributes` to platform-native line endings. | Reverting would restore CRLF on Windows and LF on Linux, matching each platform's pre-`v1.3.1` builds, but it re-breaks local Windows development (the reason `eol=lf` was introduced: vitest cannot collect a CRLF `.mjs`) and would then break anyone who installed `v1.3.1` fresh. The repair fixes every install regardless of which convention recorded it, keeps the LF policy, and is inert on healthy databases. | 2026-09-04 |
...
- **L-11:** `sqlx::migrate!()` embeds each migration file's **bytes** at compile time and validates their SHA-384 against `_sqlx_migrations` at every startup, so the checksum depends on the line endings of the *build* checkout. Adding `.gitattributes` with `* text=auto eol=lf` (commit `5df1616`, first released in `v1.3.1`) changed the Windows CI checkout from CRLF to LF, which broke every existing install with "migration 1 was previously applied but has been modified" — the SQL was byte-identical in git. Anything that changes how text files land on disk in CI is a migration-compatibility change. Fixed in `v1.3.2` by reconciling stored checksums that are provably the same SQL under the other convention (`db/mod.rs::repair_line_ending_checksums`); a checksum matching neither variant still fails loudly.
